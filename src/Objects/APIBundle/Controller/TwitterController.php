...
class TwitterController extends Controller {
    ...
    /**
     * the footer last tweets
     * @author Mahmoud
     */
    public function getLastTweetsAction($count = 2) {
        $container = $this->container;
        //get the request object
        $request = $this->getRequest();
        //create a new response for the user
        ...
        $date = new \DateTime();
        //set the response ETag
        $response->setETag($count . $date->format('dH'));
        // Check that the Response is not modified for the given Request
        if ($response->isNotModified($request)) {
            // return the 304 Response immediately
            return $response;
        }
        $tweets = TwitterController::getLastTweets($container->getParameter('consumer_key'), $container->getParameter('consumer_secret'), $container->getParameter('oauth_token'), $container->getParameter('oauth_token_secret'), $container->getParameter('twitter_user_name'), $count);
        return $this->render('ObjectsSiteBundle:Site:latest_tweets.html.twig', array(
                    'tweets' => $tweets
                        ), $response);
    }
