...
/**
 * Objects\InternJumpBundle\Entity\Banner
 * @Assert\Callback(methods={"isImageCorrect"})
 * @Assert\Callback(methods={"isFlashCorrect"})
 * @ORM\Table()
 * @ORM\HasLifecycleCallbacks
 * @ORM\Entity(repositoryClass="Objects\InternJumpBundle\Entity\BannerRepository")
 */
class Banner {
    ...
    public function isImageCorrect(ExecutionContext $context) {
        if ($this->image && !$this->url) {
            $propertyPath = $context->getPropertyPath() . '.url';
            $context->setPropertyPath($propertyPath);
            $context->addViolation('You must add url', array(), NULL);
        }
    }
